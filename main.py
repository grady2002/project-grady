...
def detectSystem():
    if os.name != "nt":
        speak("It looks like you are not using windows.")
        speak("I may not be able to deliver my best performance on non windows systems")
    else:
        pass
...
clear()
speak("hello there!")
speak("I am Olivia. your voice assistant")
detectSystem()
clear()
speak(
    "you can now leave me running in the background and i will keep responding to your voice commands."
)
...
def respond(voice_data):
    ...
    if "open documentation" in voice_data:
        speak("what documentation you want to see ?")
        doc = record_audio()
        if "complete" in doc:
            speak("opening developer documentation")
            webbrowser.get().open("https://devdocs.io/")
        if "package" in doc:
            speak("opening python project index")
            webbrowser.get().open("https://pypi.org/")
        else:
            speak("I could not understand what documentation you wanted")
